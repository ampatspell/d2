...
export class NodeModel<Model extends NodeDocumentModel = NodeDocumentModel> extends Subscribable<NodeDocumentModelLoaderOptions<Model>> {
...
export const nodeForQuery = <Model extends NodeDocumentModel = NodeDocumentModel>(ref: fs.Query, factory?: NodeDocumentModelFactory<Model>) => {
  return new NodeModel({ ref, factory });
}

export const nodeForId = <Model extends NodeDocumentModel = NodeDocumentModel>(id: string, factory?: NodeDocumentModelFactory<Model>) => {
  return nodeForQuery(fs.query(nodesCollection, fs.where(fs.documentId(), '==', id)), factory);
}

export const nodeForIdentifier = <Model extends NodeDocumentModel = NodeDocumentModel>(identifier: string, factory?: NodeDocumentModelFactory<Model>) => {
  return nodeForQuery(fs.query(nodesCollection, fs.where('identifier', '==', identifier)), factory);
};

export const nodeForPath = <Model extends NodeDocumentModel = NodeDocumentModel>(path: string, factory?: NodeDocumentModelFactory<Model>) => {
